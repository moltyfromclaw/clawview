...
function SetupPage() {
  ...
  const testConnection = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setTestResult({ ok: false, message: `❌ Error ${res.status}: ${text}` })
      }
    } catch (error) {
  ...
  const saveAgent = () => {
    try {
      ...
      agents.push({
        id: `agent-${Date.now()}`,
        name: new URL(endpointUrl).hostname.split('.')[0] || 'My Agent',
        ...
        gatewayToken: token,
        createdAt: Date.now(),
        useStatsEndpoint: true, // Flag to use /stats endpoint directly
      })
      ...
      <header className="border-b border-gray-800 bg-gray-900/50 backdrop-blur-sm">
        <div className="max-w-3xl mx-auto px-4 py-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-lg font-semibold">Connect Your OpenClaw Agent</h1>
              <p className="text-sm text-gray-400">Set up a stats endpoint for full historical data</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-3xl mx-auto px-4 py-8">
        ...
        {step === 1 && (
          <div className="space-y-6">
            <div>
              <h2 className="text-xl font-semibold mb-2">Step 1: Send Setup Prompt to Your Agent</h2>
              <p className="text-gray-400">
                Copy this prompt and send it to your OpenClaw agent. It will create a stats server 
                that exposes your full session history.
              </p>
            </div>
            ...
            <div className="bg-blue-900/20 border border-blue-800 rounded-lg p-4">
              ...
              <p className="text-sm text-gray-300">
                Your agent will create a Node.js script that reads your session files and serves 
                the data via HTTP. It will tell you the URL and password to use.
              </p>
            </div>
        ...
        {step === 2 && (
          <div className="space-y-6">
            <div>
              <h2 className="text-xl font-semibold mb-2">Step 2: Wait for Your Agent</h2>
              <p className="text-gray-400">
            ...
            <div className="bg-gray-900 rounded-lg border border-gray-800 p-4 space-y-3">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <div className="text-sm text-gray-400">Something like: http://192.168.1.100:18790</div>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <span className="text-xl">🔑</span>
                <div>
                  ...
                  <div className="text-sm text-gray-400">Your OPENCLAW_GATEWAY_PASSWORD value</div>
                </div>
              </div>
            </div>
        ...
        {step === 3 && (
          <div className="space-y-6">
            <div>
              <h2 className="text-xl font-semibold mb-2">Step 3: Enter Connection Details</h2>
              <p className="text-gray-400">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm text-gray-400 mb-1">Stats Endpoint URL</label>
                <input
                  type="text"
                  value={endpointUrl}
              ...
              <div>
                <label className="block text-sm text-gray-400 mb-1">Auth Token (OPENCLAW_GATEWAY_PASSWORD)</label>
                <input
                  type="password"
                  value={token}
        ...
        {step === 4 && (
          <div className="space-y-6">
            <div className="text-center">
              ...
              <p className="text-gray-400">
                Your OpenClaw agent is connected. Click below to add it to your dashboard.
              </p>
            </div>
        ...
        <div className="mt-12 pt-8 border-t border-gray-800">
          ...
          <div className="space-y-3 text-sm text-gray-400">
            <details className="bg-gray-900 rounded-lg p-3">
              <summary className="cursor-pointer font-medium text-gray-300">Agent didn't create the server?</summary>
              <p className="mt-2">Try sending the prompt again. Make sure your agent has file system access (write, exec tools).</p>
            </details>
            <details className="bg-gray-900 rounded-lg p-3">
              <summary className="cursor-pointer font-medium text-gray-300">Connection refused?</summary>
              <p className="mt-2">Make sure the stats server is running: <code className="bg-gray-800 px-1 rounded">ps aux | grep stats-server</code></p>
            </details>
            <details className="bg-gray-900 rounded-lg p-3">
              <summary className="cursor-pointer font-medium text-gray-300">Can't connect from outside network?</summary>
              <p className="mt-2">You'll need to expose the port via Tailscale funnel or port forwarding.</p>
            </details>
          </div>
        </div>
      </main>
